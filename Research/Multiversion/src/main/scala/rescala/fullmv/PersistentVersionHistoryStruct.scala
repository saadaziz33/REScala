...
import rescala.graph.RPValueWrappers.PersistentValue
...
case class FramingBranchOut[R](out: Set[Reactive[R]]) extends FramingBranchResult[R] {
  override def processBranching(txn: Transaction, taskPool: TaskPool): Unit = {
    if(out.size != 1) txn.branches(out.size - 1)
    out.foreach { node => taskPool.addFraming(node, txn) }
  }
}
case class FramingBranchOutSuperseding[R](out: Set[Reactive[R]], supersede: Transaction) extends FramingBranchResult[R] {
  override def processBranching(txn: Transaction, taskPool: TaskPool): Unit = {
    if(out.size != 1) txn.branches(out.size - 1)
...
class PersistentVersionHistoryStruct[V, R](val host: Host, init: Transaction, initialValue: PersistentValue[V]) extends PersistentReevaluationStruct[V, R] with PersistentAccessStruct[V, R] {
  ...
  var incomings: Set[Reactive[R]] = Set.empty
  ...
  /**
    ...
    */
  private def createVersion(position: Int, txn: Transaction, pending: Int = 0, changed: Int = 0): Version[PersistentValue[V], R] = {
    assert(position > 0)
  ...
  /**
    ...
    */
  private def prevWrite(txn: Transaction, from: Int): Version[PersistentValue[V], R] = {
    var position = from - 1
  ...
  /**
    ...
    */
  private def incrementFrame0(txn: Transaction): FramingBranchResult[R] = {
  ...
  /**
    ...
    */
  def notify(txn: Transaction, changed: Boolean, maybeFollowFrame: Option[Transaction]): NotificationResultAction[R] = synchronized {
    val position = findFrame(txn)
  ...
  override def reevIn(turn: Turn[_]): (PersistentValue[V], Set[Reactive[R]]) = {
  ...
  /**
    ...
    */
  @tailrec
  private def progressToNextWriteForNotification(out: Set[Reactive[R]], txn: Transaction, changed: Boolean): NotificationAndMaybeNextReevaluation[R] = {
  ...
  /**
    ...
    *         own writes.
    */
  def before(txn: Transaction): PersistentValue[V] = synchronized {
    @tailrec
  ...
  /**
    ...
    */
  def now(txn: Transaction): PersistentValue[V] = synchronized {
  ...
  /**
    ...
    */
  def after(txn: Transaction): PersistentValue[V] = synchronized {
    @tailrec
...
//      assert(_versions(-position - 1).out.contains(ticket.issuer), "regRead invoked without existing edge")
  ...
  /**
    ...
    */
  private def retrofitSourceOuts(position: Int, delta: Reactive[R], arity: Int): (ArrayBuffer[Transaction], Option[Transaction]) = {
