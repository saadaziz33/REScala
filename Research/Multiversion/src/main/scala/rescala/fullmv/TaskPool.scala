...
case class Notification(node: PersistentVersionHistoryStruct[_], txn: Transaction, changed: Boolean, maybeFollowFrame: Option[Transaction]) extends Task {
  def apply(): Unit = node.notify(txn, changed, maybeFollowFrame)
}
case class IncrementFrame(node: PersistentVersionHistoryStruct[_], txn: Transaction) extends Task {
  def apply(): Unit = node.incrementFrame(txn)
}
case class IncrementSupersedeFrame(node: PersistentVersionHistoryStruct[_], txn: Transaction, superseded: Transaction) extends Task {
  def apply(): Unit = node.incrementSupersedeFrame(txn, superseded)
}

trait TaskPool {
  def addFraming(node: PersistentVersionHistoryStruct[_], txn: Transaction): Unit
  def addSupersedingFraming(node: PersistentVersionHistoryStruct[_], txn: Transaction, superseded: Transaction): Unit
  def addNotification(node: PersistentVersionHistoryStruct[_], txn: Transaction, changed: Boolean, maybeFollowFrame: Option[Transaction]): Unit
}
...
class StoringTaskPool extends TaskPool {
  ...
  override def addFraming(node: PersistentVersionHistoryStruct[_], txn: Transaction): Unit = {
    framings.enqueue(IncrementFrame(node, txn))
  }
  override def addSupersedingFraming(node: PersistentVersionHistoryStruct[_], txn: Transaction, superseded: Transaction): Unit = {
    supersedingFramings.enqueue(IncrementSupersedeFrame(node, txn, superseded))
  }
  override def addNotification(node: PersistentVersionHistoryStruct[_], txn: Transaction, changed: Boolean, maybeFollowFrame: Option[Transaction]): Unit = {
